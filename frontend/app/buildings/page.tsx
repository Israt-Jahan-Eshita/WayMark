  ...
  return (
    <div className="w-full max-w-4xl mx-auto pt-28 pb-16 px-4">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-3xl font-bold text-[var(--color-foreground)] mb-2">{t("audited_locations", "Audited Locations")}</h1>
        <p className="text-[var(--text-secondary)] text-sm mb-6">{t("audited_locations_desc", "View accessibility history for all scanned locations.")}</p>
        ...
        <div className="relative max-w-md group">
          ...
          <input
            ...
            placeholder={t("filter_by_building", "Filter by building name or location...")}
            className="w-full neu-input py-3 pl-12 pr-10 text-sm font-medium focus:ring-0"
          />
      ...
      <div className="grid gap-4">
        {filteredBuildings.length === 0 ? (
          <div className="neu-panel p-8 text-center text-[var(--text-secondary)] font-medium text-sm">
            {buildings.length === 0 ? t("no_buildings_audited", "No buildings have been audited yet.") : t("no_buildings_match", "No buildings match your search.")}
          </div>
        ) : (
                ...
                <div className="neu-panel p-4 flex flex-col md:flex-row justify-between items-start md:items-center group hover:border-[var(--color-primary)]/50 transition-colors cursor-pointer">
                  ...
                  <div className="mt-3 md:mt-0 flex items-center gap-4 w-full md:w-auto justify-between shrink-0 pl-14 md:pl-0">
                    <div className="flex flex-col items-start md:items-end">
                      <span className="text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-bold">{t("latest_score", "Latest Score")}</span>
                      <span className="text-xl font-black text-[var(--color-primary)]">
                        {building.latest_score || t("not_available", "N/A")}
                      </span>
                    </div>
